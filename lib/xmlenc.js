...
function decryptKeyInfo(doc, options) {
  ...
  switch (keyEncryptionAlgorithm) {
    case 'http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p':
      return decryptKeyInfoWithRSAOAEPScheme(encryptedKey, options, 'RSA-OAEP');
    case 'http://www.w3.org/2001/04/xmlenc#rsa-1_5':
      return decryptKeyInfoWithRSAV15Scheme(encryptedKey, options, 'RSAES-PKCS1-V1_5');
    default:
      throw new Error('key encryption algorithm ' + keyEncryptionAlgorithm + ' not supported');
  }
}

function decryptKeyInfoWithRSAOAEPScheme(encryptedKey, options, scheme) {
...
function decryptKeyInfoWithRSAV15Scheme(encryptedKey, options, scheme) {
  const privateKey = forge.pki.privateKeyFromPem(options.key.toString());
  const cipherValueBytes = forge.util.decode64(encryptedKey.textContent.replace(/\s+/g, ''));
  const padding_pkcs = scheme === 'RSA-OAEP' ? 'RSA-OAEP' : 'RSAES-PKCS1-V1_5';

  const decryptedBytes = privateKey.decrypt(cipherValueBytes, padding_pkcs);
